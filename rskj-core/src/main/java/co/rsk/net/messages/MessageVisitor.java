...
public class MessageVisitor {
  ...
  /**
   * Processes a BlockMessage message, adding the block to the blockchain if
   * appropriate, or
  ...
  public void apply(BlockMessage message) {
    blockPropagationProfiler.info("{}",
        new BlockProfilingData(message.getBlock().getHash().getBytes(), Phase.PROCESSING_START));
    ...
    if (result.isInvalidBlock()) {
      reportEventToPeerScoring(sender, EventType.INVALID_BLOCK, "Invalid block {} {} at {}", blockNumber,
          block.getPrintableHash(), this.getClass());
      return;
    }
    ...
    reportEventToPeerScoring(sender, EventType.VALID_BLOCK, "Valid block {} {} at {}", blockNumber,
        block.getPrintableHash(), this.getClass());
  }
  ...
  public void apply(TransactionsMessage message) {
    ...
    for (Transaction tx : messageTxs) {
      if (!tx.acceptTransactionSignature(config.getNetworkConstants().getChainId())) {
        reportEventToPeerScoring(sender, EventType.INVALID_TRANSACTION, "Invalid transaction {} at {}",
            tx.getHash().toString(), this.getClass());
      } else {
        txs.add(tx);
        reportEventToPeerScoring(sender, EventType.VALID_TRANSACTION, "Valid transaction {} at {}",
            tx.getHash().toString(), this.getClass());
      }
    }
    ...
    if (loggerMessageProcess.isDebugEnabled()) {
      loggerMessageProcess.debug("Tx message process finished after [{}] seconds.",
          FormatUtils.formatNanosecondsToSeconds(System.nanoTime() - start));
    }
  }
  ...
  private void tryRelayBlock(Block block, BlockProcessResult result) {
    ...
    if ((result.isScheduledForProcessing() || result.wasBlockAdded(block))
        && !this.blockProcessor.hasBetterBlockToSync()) {
      relayBlock(block);
    }
  }
  ...
  private void relayBlock(Block block) {
    ...
    blockPropagationProfiler.info("{}", new BlockProfilingData(block.getHash().getBytes(), Phase.BROADCASTED));
